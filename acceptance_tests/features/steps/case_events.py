...
@step('The rerun UAC Updated events are emitted, matching the types that were emitted on sample load')
def gather_uac_updated_events(context):
    expected_qid_types_for_cases = defaultdict(list)
    for uac_created_event in context.uac_created_events:
        expected_qid_types_for_cases[uac_created_event['payload']['uac']['caseId']].append(
            uac_created_event['payload']['uac']['questionnaireId'][:2])

    start_listening_to_rabbit_queue(Config.RABBITMQ_RH_OUTBOUND_UAC_QUEUE,
                                    functools.partial(store_all_uac_updated_msgs_by_collection_exercise_id,
                                                      context=context,
                                                      expected_msg_count=len(context.uac_created_events),
                                                      collection_exercise_id=context.collection_exercise_id))
    test_helper.assertEqual(len(context.messages_received), len(context.uac_created_events))
    uac_updates = context.messages_received.copy()
    context.messages_received = []

    actual_qid_types_for_cases = defaultdict(list)
    for uac_update in uac_updates:
        actual_qid_types_for_cases[uac_update['payload']['uac']['caseId']].append(
            uac_update['payload']['uac']['questionnaireId'][:2])
    test_helper.assertEqual(sorted(actual_qid_types_for_cases), sorted(expected_qid_types_for_cases),
                            'The QID types by case generated by the rerun initial contact'
                            ' print files should match the types generated on sample load')

    # Reassign the uac created events so that further steps use these UACs instead of the sample load ones
    context.uac_created_events = uac_updates


@step('UAC Updated events are emitted for the cases with matching treatment codes'
