...
def check_correct_files_on_sftp_server(context):
    ...
    _check_notification_files_have_all_the_expected_data(context.test_start_datetime,
                                                         context.survey_ref,
                                                         context.period,
                                                         expected_csv_lines)


def _check_notification_files_have_all_the_expected_data(start_of_test, survey_ref, period,
                                                         expected_csv_lines):
    logger.debug('Checking for files on SFTP server')

    with SFTPUtility() as sftp_utility:
        abc(sftp_utility, survey_ref, period, start_of_test, expected_csv_lines)


@retry(retry_on_exception=lambda e: isinstance(e, FileNotFoundError),
       wait_fixed=5000, stop_max_attempt_number=24)
def abc(sftp_utility, survey_ref, period,
        start_of_test, expected_csv_lines):
